    ...
    function buildUI(thisObj) {
        ...
        function baixarFotosDosLinks(textoLinks) {
            ...
            for (var j = 0; j < total; j++) {
                ...
                var caminhoArquivo = pasta.fsName + "\\" + nomeArquivo;

                var comando = 'cmd.exe /c curl.exe -L "' + url + '" --output "' + caminhoArquivo + '"';

                try {
                    system.callSystem(comando);

                    var arquivoBaixado = new File(caminhoArquivo);
                    if (arquivoBaixado.exists && arquivoBaixado.length > 0) {
                        baixados++;
                    } else {
                        falhas.push((j + 1) + " -> " + url);
                    }
        ...
        var abrirLinksBtn = win.add("button", undefined, "Links das Fotos");
        abrirLinksBtn.alignment = ["fill", "top"];
        ...
        abrirLinksBtn.onClick = function () {
            ...
            baixarBtn.onClick = function () {
                ...
                baixarFotosDosLinks(linksTexto);
            };
