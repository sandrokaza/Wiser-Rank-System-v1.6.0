    ...
    function buildUI(thisObj) {
        ...
        function coletarCompsPorNome() {
            ...
            for (var i = 1; i <= app.project.numItems; i++) {
                ...
                var nomeNormalizado = normalizarNomeBase(item.name);

                if (nomeNormalizado === "categoria") {
                    categoriaComp = item;
                    continue;
                }

                if (/^foto\s*\d+$/i.test(nomeNormalizado)) continue;

                var pos = getOrdemNome(nomeNormalizado);
                ...
                if (nomeNormalizado.indexOf("tag") !== -1) {
                    mapa[pos].tag = item;
                } else {
                    mapa[pos].name = item;
                }
        ...
        applyBtn.onClick = function () {
            ...
            var fallback = coletarCompsPorNome();
            ...
            for (var i = 1; i <= 20; i++) {
                ...
                if (!compNome && fallback.mapa[i] && fallback.mapa[i].name) {
                    compNome = fallback.mapa[i].name;
                }

                if (!compTag && fallback.mapa[i] && fallback.mapa[i].tag) {
                    compTag = fallback.mapa[i].tag;
                }
            ...
            if (!categoriaComp) {
                categoriaComp = fallback.categoria;
            }
        ...
        criarIdsBtn.onClick = function () {
            ...
            var dados = coletarCompsPorNome();
            var mapa = dados.mapa;
            var criadas = 0;

            for (var i = 1; i <= 20; i++) {
                if (!mapa[i]) {
                    mapa[i] = { name: null, tag: null };
                }
            }
            ...
            alert("IDs criadas corretamente por posição.\nTotal de IDs criadas: " + criadas);
        };
